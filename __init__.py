...
class AVM(SmartPlugin):
    ...
    def _get_post_request(self, url, data, headers):
        ...
        # if str(response) == '<Response [200]>':
            # self.logger.debug(f' _get_post_request successful')
        # else:
            # self.logger.warning(f'_get_post_request successful failed')
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        ...
        if caller.lower() != 'avm':
            if self.get_iattr_value(item.conf, 'avm_data_type') == 'wlanconfig':
                ...
                self.logger.debug(f"switch to be set to: {cmd_switch_state}")
                ...
                self.logger.debug(f"switch to be set to: {cmd_switch_toggle}")
    ...
    def set_wlanconfig(self, wlan_index, new_enable=False):
        param = "%s%s%s" % ("/upnp/control/", 'wlanconfig', str(wlan_index))
        url = self._build_url(param)
        headers = self._header.copy()
        action = 'SetEnable'
        headers['SOAPACTION'] = "%s#%s" % (self._urn_map['WLANConfiguration'] % str(wlan_index), action)
        soap_data = self._assemble_soap_data(action, self._urn_map['WLANConfiguration'] % str(wlan_index), {'NewEnable': int(new_enable)})
    ...
    def _update_smarthome_items(self):
        for item in self._fritz_device._smarthome_items:
            ...
            # set item value for relevant read-only items #Dict-Key muss dem jewiligen avm_data_type entsprechen
    ...
    def _update_wan_common_interface_configuration(self, item):
        ...
        if "wan_common_interface_configuration_" + action not in self._response_cache:
            ...
            self.logger.debug(
                "Accessing wan_common_interface_configuration response cache for action %s and item %s!" % (action, item.property.path))
    ...
    def set_deflection(self, deflection_id=0, new_enable=False):
        ...
        # if str(response) == '<Response [200]>':
        #     self.logger.debug(f'set_deflection successfully set deflection_id <{deflection_id}> to {new_enable}')
        # else:
        #     self.logger.warning(f'set_deflection set deflection_id <{deflection_id}> to {new_enable} failed')
